...
$(document).ready(function() {
    ...
    $('.select-button').click(function(){
    	if ($('button.select-button').html() == '<?php echo lang::get("Batal")?>'){
    		$('td input[type="checkbox"]').each( function() {$(this).attr("checked", null);});
    		$('button.select-button').html('<?php echo lang::get("Pilih Semua")?>');
    	
    	}else{
    		$('td input[type="checkbox"]').each( function() {$(this).attr("checked", status);});
    		$('button.select-button').html('<?php echo lang::get("Batal")?>');
    	}
    });
    ...
    $('.delete-selected').click(function(){
    	...
    	$('#modal').html(' ');
    	var output = '<p class="lead"><?php echo lang::get("Yakin Hapus File Ini?")?></p><hr />';
    	output += '<div class="modal-buttons right">';
    	output += '<button id="confirm-button" type="button" class="nice alert radius button"><?php echo lang::get("Hapus")?></button>';
    	output += '<button id="cancel-button" type="button" class="nice radius button"><?php echo lang::get("Cancel")?></button>';
    	output += '</div>';
    	output += '<a class="close-reveal-modal"></a>';
    ...
    $('.action-info').click(function(){
    	...
    	// cancel sub-button
    	output += '<button id="cancel-button" type="button" class="nice radius button"><?php echo lang::get("Kembali")?></button>';
    	output += '</div>';
    	output += '<a class="close-reveal-modal"></a>';
    	...
    	$('#email-button').click(function(){
    		...
    		output += '<button id="second-cancel-button" type="button" class="nice radius button"><?php echo lang::get("Kembali")?></button>';
    		output += '</div>';
    ...
    $('.action-account').click(function(){
    	...
    	if (data_username == '-1'){
    		...
    		var is_new = '';
	    	var password_fieldname = '<?php echo lang::get("Ubah Password ")?>';
    	...
    	$('#modal').html(' ');
   		...
   		if (data_username == 'admin' || data_username == 'guest'){
   			...
   			output += '<h5><?php echo lang::get("Tambah User")?></h5><hr />';
   		}else{
    	...
    	if (data_username != 'admin'){
    		...
    		output += '<h5><?php echo lang::get("Hak Izin :")?></h5><input type="checkbox" '+p_read+' id="read"><?php echo lang::get("Read")?> <input type="checkbox" '+p_write+' id="write"><?php echo lang::get("Write")?> <input type="checkbox" '+p_upload+' id="upload"><?php echo lang::get("Upload")?>';
    	}
    	
    	output += '</div>';
    	
    	output += '<div class="modal-buttons right">';
    	output += '<button id="save-button" type="button" class="nice radius button"><?php echo lang::get("Simpan")?></button>';
    	output += '</div>';
    	...
    	$('#save-button').click(function(){
			...
			$.post("./?account="+data_username, { is_new: is_new, homedir: homedir, email: email, read: read, write: write, upload: upload, password: set_password} ).done(function(data) {
				// flush 
    ...
    $('.action-delete-account').click(function(){
    	...
    	$('#modal').html(' ');
    	var output = '<p class="lead"><?php echo lang::get("Apakah anda yakin ingin menghapus user ?")?></p><hr />';
    	output += '<div class="modal-buttons right">';
...
function isSelected(){
	if ($('td input[type="checkbox"]:checked').is(":empty") == false){
		...
		$('#modal').html(' ');
		$('#modal').append('<p class="lead"><?php echo lang::get("Tidak Ada File Yang Dipilih.")?></p><hr />');
		$('#modal').append('<button id="cancel-button" type="button" class="nice radius button right"><?php echo lang::get("Kembali")?></button>');
